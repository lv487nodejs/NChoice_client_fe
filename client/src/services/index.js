...
export default class StoreService {
  ...
  getCatalogById = (id) => {
    const catalogs = this.getResource(`catalogs/${id}`);
    return catalogs;
  };
  ...
  postComments = (comment) => {
    const res = this.postData('comments', comment);
    return res;
  };

  deleteComment = (id, token) =>
    axios.delete(`${_apiBase}comments/${id}`, {
      headers: { 'x-auth-token': token }
    });

  confirmEmail = (token) => {
    const res = this.getResource(`auth/confirmation/${token}`);
    return res;
  };

  updateRate = (id, userId, rate, token) =>
    axios.put(
      `${_apiBase}rating/${id}`,
      { rate, userId },
      { headers: { 'x-auth-token': token } }
  ...
  oauthFacebook = (token) => {
    const res = this.postData('auth/oauth/facebook', token);
    return res;
  };
}
