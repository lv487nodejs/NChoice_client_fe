...
const realPrice = `${price * currency} ${currencyIcon}`;
const msrpPrice = `${msrp * currency} ${currencyIcon}`;
...
describe('Visit product list page', () => {
  ...
  it('Check product image, rate, name', () => {
    ...
    cy.get('#productImg').should(
      'have.attr',
      'src',
      `/images/products/${imgAlt}`
    );
  ...
  it('Check product price', () => {
    ...
    cy.get('#realPrice').should('have.text', realPrice);
    cy.get('#msrpPrice').should('have.length', 1);
    cy.get('#msrpPrice').should('have.text', msrpPrice);
  });
});
