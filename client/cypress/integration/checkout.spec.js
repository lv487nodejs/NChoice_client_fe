...
describe('check root for checkout', () => {
  it('add to cart and visit checkout', () => {
    ...
    cy.get('#buyNow').should('not.be.visible');
    cy.get('.size > :nth-child(1) > span').wait(50).click();
    cy.get('#buyNow')
  ...
  it('check all elements (header)', () => {
    cy.get('h2.text-center').should('exist').and('be.visible');
  });
...
describe('add to many items', () => {
  it('check table before adding items', () => {
    cy.get('tbody > :nth-child(1) > :nth-child(3)').should('contain', '1');
  });
...
describe('fill in form', () => {
  it('fill all fields', () => {
    ...
    cy.get('#emailValidate').type(form.email).should('have.value', form.email);
    cy.get('#phoneValidate')
      .type(form.contactPhone)
      .should('have.value', form.contactPhone);
    cy.get('#countryValidate')
      .select(form.country)
      .should('have.value', form.country);
    cy.get('#cityValidate').type(form.city).should('have.value', form.city);
    cy.get('#streetValidate')
      .type(form.street)
      .should('have.value', form.street);
    cy.get('#buildingValidate')
      .type(form.buildingNumber)
      .should('have.value', form.buildingNumber);
    cy.get('#deliveryTypeValidate')
      .select(form.deliveryType)
      .should('have.value', form.deliveryType);
    cy.get('#paymentMethodValidate')
...
describe('check form values still in after changing page', () => {
  it('check inputs', () => {
    cy.get('a > .btn').click();
    cy.get('[href="/checkout"] > .cart-btns').click();
    cy.get('#firstNameValidate').should('have.value', form.firstName);
    cy.get('#lastNameValidate').should('have.value', form.lastName);
    cy.get('#emailValidate').should('have.value', form.email);
    cy.get('#phoneValidate').should('have.value', form.contactPhone);
    cy.get('#cityValidate').should('have.value', form.city);
    cy.get('#streetValidate').should('have.value', form.street);
    cy.get('#buildingValidate').should('have.value', form.buildingNumber);
  });
});
