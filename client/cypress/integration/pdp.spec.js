import {
  wishListArray,
  productsCollection
} from '../fixtures/productList/products';
...
describe('Check information about product', () => {
  ...
  it('Check buttons', () => {
    ...
    cy.get('#heartButton')
      .click()
      .should(() => {
        expect(localStorage.getItem('wishlist-collection')).to.eq(
          wishListArray
        );
      });
    cy.get('#addToCartButton').should('exist');
    cy.get('#addToCartButton').should(
      'have.attr',
      'class',
      'button disabled btn btn-dark'
    );
    ...
    cy.get('#addToCartButton')
      .click()
      .should(() => {
        expect(localStorage.getItem('products-collection')).to.eq(
          productsCollection
        );
