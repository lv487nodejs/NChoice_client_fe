...
const { catalog, category, brand, color, title, description, mrsp, price } = readyModel;
...
describe('Testing page for adding new product to db.', () => {
    it('State of productModel should be initial', () => {
        ...
        cy.route('GET', '/categories', 'fixture:productAdd/categories').as('categories');
    ...
    it('Should save product, and open it in product detail page', () => {
        cy.server();
        cy.route('POST', '/products', { _id: productId, ...readyModel }).as('post-product');
        cy.route('GET', '/products/newproduct', 'fixture:productAdd/newProduct').as('get-product');
