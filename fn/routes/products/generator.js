  ...
  try {
    setInterval(async () => {
      try {
        ...
        const condition = catalog.categories.findIndex(
          (valueId) => valueId.toString() === category.id
        );
        ...
        const rateR = parseInt(
          rateChance * chance.pick([0.8, 0.6, 0.7, 0.9], 1)
        );

        const props = [
          {
            size:
              requestedCategory.category === 'shoes'
                ? sizeShoesSmall
                : sizeWearSmall,
        ...
        const gentitle = `${requestedCategory.category} ${
          requestedBrand.brand
        } ${chance.sentence({ words: 3 }).toLowerCase()}`;
