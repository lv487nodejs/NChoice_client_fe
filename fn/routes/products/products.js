...
router.get('/', async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send({ products: productsToSend, pagesCount, foundProductsNumber });
  } catch (err) {
...
const getFilters = async (query) => {
  ...
  try {
    if (catalog) {
      const catalogItems = await Catalogs.find({
        catalog: { $in: catalog.split(',') },
      });
      catalogItems.forEach((value, i, array) => (array[i] = value.id));
      filter.catalog = { $in: catalogItems };
    }
    if (category) {
      const categoryItems = await Categories.find({
    ...
    if (brand) {
      const brandItems = await Brands.find({
    ...
    if (color) {
      const colorFilter = await Colors.find({
